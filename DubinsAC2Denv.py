...
class DubinsAC2Denv(gym.Env):
    ...
    def __init__(self, actions='discrete'): # using discrete action space for air combat scenario
        self._load_config()
        self.viewer = None # related to rendering the episode


        self._vel_mps = 20 # velocity in terms of pixel/s 
        self._action_time_s = 0.25 # time step in environment
    ...
    def step(self, action): # by using input action, environment returns an observation and reward
    ...
    def _get_sta_env_v2(self): # returns state of blue aircraft which is the input of DQN
