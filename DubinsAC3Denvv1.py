...
class DubinsAC3Denvv1(gym.Env):
    ...
    def __init__(self, actions='cont'):
        ...
        lowlim=np.array([-self.area_width,  # err_x
                         -self.area_height,  # err_y
                         -self.area_z,  # err_z
                         -180,  # LOSxy
                         -180,  # LOSz
                         -180,  # ATAxy
        ...
        highlim=np.array([self.area_width,  # err_x
                          self.area_height,  # err_y
                          self.area_z,  # err_z
                          180,  # LOSxy
                          180,  # LOSz
        ...
        if actions == 'discrete':
            ...
            self.action_space = spaces.Box( low=np.array([-1., -1., -1.]), high=np.array([1., 1., 1.]), shape=(3,), dtype=np.float32 )  # 0 -> 4 bank angle command: -90 45 0 45 90
    ...
    def step(self, action):
        ...
        cmd_bank_deg = action[0] * 90.
        cmd_bank_deg = np.clip( cmd_bank_deg, -90, 90)

        cmd_theta_deg = action[1] * 70.
        cmd_theta_deg = np.clip( cmd_theta_deg, -90, 90)

        # max: 0.7M -> 247 m/s, min: 0.18M 61 m/s
        cmd_vel_mps = (action[2] * 60) + self._blueAC._vel_mps
        cmd_vel_mps = np.clip( cmd_vel_mps, 61, 247)

        self._blueAC.takeaction(cmd_bank_deg, cmd_theta_deg, cmd_vel_mps, self._action_time_s)

        self._redAC.takeaction(0, 0, self._vel_mps, self._action_time_s)

        if self._redAC._pos_m[0] > self.area_width:
            self._redAC._heading_rad = np.mod(self._redAC._heading_rad - np.pi/2, 2*np.pi)
        elif self._redAC._pos_m[0] < 0:
            self._redAC._heading_rad = np.mod(self._redAC._heading_rad + np.pi / 2, 2 * np.pi)

        if self._redAC._pos_m[1] > self.area_height:
            self._redAC._heading_rad = np.mod(self._redAC._heading_rad - np.pi/2, 2*np.pi)
    ...
    def render(self, mode='human'):
        ...
        jtransform = rendering.Transform(rotation= -att[2], translation= np.array([pos[1] * scaleY, pos[0] * scaleX]))
        red_ac_img.add_attr(jtransform)
        self.viewer.onetime_geoms.append(red_ac_img)
        self.viewer.draw_polyline(pos_hist_scale[ ::5 , [-2, -3]],).set_color(1,0,0)

        transform2 = rendering.Transform(translation=(self.goal_pos[1] * scaleY, self.goal_pos[0] * scaleX))  # Relative offset
        self.viewer.draw_circle(50*scaleX).add_attr(transform2)

        transform2 = rendering.Transform(translation=(self.goal_pos[1] * scaleY, self.goal_pos[0] * scaleX))  # Relative offset
        self.viewer.draw_circle(300*scaleX,filled=False).add_attr(transform2)

        transform3 = rendering.Transform(translation=(pos[1] * scaleY, pos[0] * scaleX))  # red dangerous circle
        self.viewer.draw_circle(2000*scaleX,filled=False).add_attr(transform3)
        ...
        self.viewer.draw_polyline(pos_hist_scale[ ::5 , [-2, -3]]).set_color(0,0,1)
    ...
    def _load_config(self):
        # input dim
        self.area_width  = 10 * 1000 # 10km Config.window_width
        self.area_height = 10 * 1000 # 10km Config.window_height
        self.area_z = 9144 # 30k ft Config.area_z
    ...
    def _random_pos(self):
        pos0 = np.array([self.area_width / 4, self.area_height / 4, self.area_z / 4])

        return (pos0 + np.random.uniform(low=np.array([0, 0, 0]), high=np.array([self.area_width / 2, self.area_height / 2, self.area_z / 2])),
                np.random.uniform(low=-180, high=180))

    def _random_pos2(self):
        return (np.random.uniform(low=np.array([0, 0, 0]), high=np.array([self.area_width * 0.5, self.area_height * 0.5, self.area_z * 0.5])),
                np.random.uniform(low=-180, high=180))
    ...
    def _get_sta_env_v2(self):
        ...
        target_dist = np.array([self.target_point, 0., 0.])
        ...
        return np.array([self.errPos[0]/self.area_width,
                         self.errPos[1]/self.area_height,
                         self.errPos[2]/self.area_z,
                         self.LOSxy_deg,
                         self.LOSz_deg,
                         self.ATAxy_deg,
                         self.ATAz_deg,
