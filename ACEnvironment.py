...
class ACEnvironment2D:
    ...
    _dt = 0.05  # seconds,  simulation time step
    _t = 0.
	...
	# while loop ensures that action is executed for training time step
        while self._t < (startTime + Time_s): # to  achieve next state run this loop for time_s duration
    ...
    def takeaction(self, cmd_bank_deg, cmd_flightpath, cmd_vel_mps, actiontime=1):
        ...
        return self.simfor(actiontime)  # sim for training time step to get the results of an action
